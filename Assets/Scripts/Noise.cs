...
public static class Noise
{
    ...
    public static float[,] GenerateNoiseMap(int mapWidht, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, NormaliseMode normaliseMode) {
        ...
        // normalising the noise map
        for (int y = 0; y < mapHeight; y++) {
            for (int x = 0; x < mapWidht; x++) {
                if (normaliseMode == NormaliseMode.Local) {
                    noiseMap[x,y] = Mathf.InverseLerp(minLocalNoiseHeight, maxLocalNoiseHeight, noiseMap[x,y]);
                } else {
                    float normalisedHeight = (noiseMap[x, y] + 1) / (2f * maxPossibleHeight / 2.2f);
